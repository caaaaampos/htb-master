...
1. Make sure you’ve completed the prerequisite steps in the [Quickstart](/v4/quickstart). Then verify the ADB connection with `adb devices`. This should list your phone as “device” along with its Serial Number. If the phone appears as `unauthorized` make sure to accept the prompt on your phone.
...
The Docker image uses the `htb_agent` entrypoint, which means you can use any of the CLI commands described in the [CLI Usage](/v4/guides/cli) section and append it to the following base command:
