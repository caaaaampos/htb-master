...
**For CLI usage only:**
```bash
uv tool install htb_agent
```

**For CLI + Python integration:**
```bash
uv pip install htb_agent
```

<Note>
Most LLM providers (Google Gemini, OpenAI, Ollama, OpenRouter) are included by default. For additional providers, install extras: `uv pip install 'htb_agent[anthropic,deepseek]'`.
</Note>

Note: `htb_agent` now provides a `--version` flag — verify your installation with `htb_agent --version` after installing.
...
**v3 Code:**
```python
from htb_agent import AdbTools, DroidAgent
...
uv pip install 'htb_agent[anthropic,deepseek]'
```
...
from htb_agent import DroidAgent
from htb_agent.config_manager import DroidrunConfig
...
htb_agent run "Send an email to john@example.com" --reasoning
```
...
```python
from htb_agent import DroidAgent
from htb_agent.config_manager import DroidrunConfig, TracingConfig, LoggingConfig
...
htb_agent run "Open Settings" --tracing

# Enable trajectory recording (screenshots + UI state)
htb_agent run "Open Settings" --save-trajectory step
```
