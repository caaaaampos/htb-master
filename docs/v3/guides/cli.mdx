...
# HTB Agent CLI Guide: Using OpenAILike, Ollama, and Gemini Providers

HTB Agent lets you control Android devices using natural language and LLM agents. This guide explains how to use the CLI with three popular LLM providers: **OpenAILike**, **Ollama**, and **Gemini**.
...
<Steps>
    <Step title="Install HTB Agent and its dependencies. Choose which ever provider you'd like to use.">
       ```sh 
       pip install 'htb_agent[google,anthropic,openai,deepseek,ollama,dev]'
       ```
    </Step>
    <Step title="Ensure your Android device is connected and the HTB Agent Portal is installed">
...
```sh
htb_agent run "<your natural language command>" [OPTIONS]
```
...
Here you can see a bunch of provider specific htb_agent cli usage examples
...
DeepSeek is a powerful LLM provider that can be used with HTB Agent for natural language Android automation.
...
- Use `htb_agent devices` to list connected devices.
...
- [HTB Agent GitHub](https://github.com/htb_agent/htb_agent)
