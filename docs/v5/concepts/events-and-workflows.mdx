...
HTB Agent provides **real-time event streaming** that gives you visibility into agent execution as it happens. This allows you to build UIs, logging systems, or monitoring tools that react to agent actions in real-time.

Under the hood, HTB Agent uses [llama-index workflows](https://docs.llamaindex.ai/en/stable/understanding/workflows/) - an event-driven orchestration system that powers the agent architecture.
...
```python
# Common events (htb_agent/agent/common/events.py)
...
logger = logging.getLogger("htb_agent.monitor")
...
- Located in `htb_agent/agent/droid/events.py`
