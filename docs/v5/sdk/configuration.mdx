...
```python
from htb_agent import DroidAgent, DroidrunConfig
...
```python
from htb_agent import AgentConfig, FastAgentConfig, ManagerConfig, ExecutorConfig, ScripterConfig, AppCardConfig

AgentConfig(
    # Core settings
    name="htb_agent",                 # Agent name (use "htb_agent" for default agent)
...
```python
from htb_agent import DeviceConfig
...
```python
from htb_agent import LoggingConfig
...
```python
from htb_agent import TracingConfig
...
```python
from htb_agent import TelemetryConfig
...
```python
from htb_agent import ToolsConfig
...
```python
from htb_agent import CredentialsConfig
...
```python
from htb_agent import SafeExecutionConfig
...
### Config Format
```python
from htb_agent import DroidrunConfig, CredentialsConfig
...
htb_agent run "Task" --steps 30 --reasoning --vision

# Override device
htb_agent run "Task" --device emulator-5554 --tcp

# Override LLM (applies to ALL agents)
htb_agent run "Task" --provider GoogleGenAI --model models/gemini-2.5-flash

# Override logging
htb_agent run "Task" --debug --save-trajectory action --tracing

# Custom config file
htb_agent run "Task" --config /path/to/config.yaml
```
