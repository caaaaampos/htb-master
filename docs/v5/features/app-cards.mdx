...
htb_agent run "Archive all unread emails" --reasoning

# App cards not used (direct execution mode)
htb_agent run "Tap the button"
```
...
```bash
htb_agent run "Open Chrome and search for htb_agent" --reasoning --debug
```
...
5. **Are you using reasoning mode?**
   ```bash
   htb_agent run "command" --reasoning
   ```
...
```bash
htb_agent run "Open Gmail" --reasoning --debug
```
