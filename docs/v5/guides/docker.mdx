...
1. Make sure you’ve completed the prerequisite steps in the [Quickstart](/v5/quickstart). Then verify the ADB connection with `adb devices`. This should list your phone as “device” along with its Serial Number. If the phone appears as `unauthorized` make sure to accept the prompt on your phone.
...
```bash
docker pull ghcr.io/htb_agent/htb_agent:latest
```
...
| `--volume ~/.android:/home/htb_agent/.android` | Mounts the host’s Android configuration directory into the container. | Stores ADB keys and settings so that once you grant permission on the host, Droidrun doesn’t prompt for it again inside the container. |

The Docker image uses the `htb_agent` entrypoint, which means you can use any of the CLI commands described in the [CLI Usage](/v5/guides/cli) section and append it to the following base command:
...
docker run \
  ...
  --volume ~/.android:/home/htb_agent/.android \
  ghcr.io/htb_agent/htb_agent:latest \
  <htb_agent command>
```
...
docker run --group-add plugdev --device /dev/phone1/phone:/dev/phone --volume /dev/bus/usb:/dev/bus/usb --volume ~/.android:/home/htb_agent/.android --env GOOGLE_API_KEY=your-api-key-here ghcr.io/htb_agent/htb_agent:latest run "Open the settings app and tell me the Android version"

# Override provider and model
docker run --group-add plugdev --device /dev/phone1/phone:/dev/phone --volume /dev/bus/usb:/dev/bus/usb --volume ~/.android:/home/htb_agent/.android --env OPENAI_API_KEY=your-api-key-here ghcr.io/htb_agent/htb_agent:latest run "Open a browser and search for htb_agent" -p OpenAI -m gpt-4o

# Use a locally-running LLM, example for LM Studio running in the LAN
docker run --group-add plugdev --device /dev/phone1/phone:/dev/phone --volume /dev/bus/usb:/dev/bus/usb --volume ~/.android:/home/htb_agent/.android --network host ghcr.io/htb_agent/htb_agent:latest run "Open a browser and search for htb_agent" -p OpenAILike -m gpt-oss --api_base http://IP-of-LMStudio-server:PORT/v1
```
