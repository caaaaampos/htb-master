...
<Steps>
  ...
  <Step title="Install Portal App">
    ...
    # Or specify device
    htb_agent setup --device SERIAL_NUMBER
    ```
...
<AccordionGroup>
  <Accordion title="TCP Mode (Recommended) - per operation">
    ...
    **Enable:**
    ```bash
    # CLI
    htb_agent run "your command" --tcp
...
<AccordionGroup>
  ...
  <Accordion title="Wireless Debugging (Android 10 and Below)">
    <Steps>
      ...
      <Step title="Connect Wirelessly">
        ...
        htb_agent ping --device DEVICE_IP:5555
        ```
      </Step>
    </Steps>
  </Accordion>
  ...
  <Accordion title="Multiple Devices">
    ...
    ```bash
    # CLI
    htb_agent run "your command" --device emulator-5554
    ...
    ```python
    import asyncio
    from htb_agent import DeviceConfig, DroidrunConfig, DroidAgent
...
<AccordionGroup>
  ...
  <Accordion title="Portal not installed">
    **Symptoms:** `htb_agent ping` fails with "Portal is not installed"

    **Solutions:**
    1. Reinstall: `htb_agent setup`
    2. Check: `adb shell pm list packages | grep htb_agent`
    3. Verify APK architecture matches device (arm64-v8a for most devices)
  </Accordion>

  <Accordion title="Accessibility service not enabled">
    **Symptoms:** `htb_agent ping` fails with "accessibility service not enabled"
  ...
  <Accordion title="Empty UI state">
    ...
    **Solutions:**
    1. Verify accessibility: `htb_agent ping`
...
<Steps>
  ...
  <Step title="Test Connection">
    ...
    ```bash
    htb_agent run "take a screenshot" --ios
    ```

    <Note>
    The `htb_agent ping` command is Android-only (uses ADB). For iOS, run a simple task with `--ios` to verify the connection.
    </Note>
  </Step>
</Steps>
...
| Setup Tool | `htb_agent setup` | Manual installation |
...
```python
from htb_agent import DroidAgent, DroidrunConfig, DeviceConfig
...
```bash
# Use --ios flag
htb_agent run "your command" --ios
