...
<Note>
Most LLM providers (Google Gemini, OpenAI, Ollama, OpenRouter) are included by default. For additional providers, install extras: `uv tool install 'htb_agent[anthropic,deepseek]'`.
</Note>
...
htb_agent run "Open the settings app and tell me the Android version"

# Override provider and model
htb_agent run "Check the battery level" --provider OpenAI --model gpt-4o

# Enable vision mode (sends screenshots to LLM)
htb_agent run "What app is currently open?" --vision

# Enable reasoning mode (uses Manager-Executor workflow for complex tasks)
htb_agent run "Find a contact named John and send him an email" --reasoning
```
