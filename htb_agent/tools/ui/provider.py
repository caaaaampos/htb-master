...
if TYPE_CHECKING:
    ...
    from htb_agent.tools.filters import TreeFilter
    from htb_agent.tools.formatters import TreeFormatter
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "error" in combined_data:
                    err_type = combined_data.get("error", "")
                    err_msg = combined_data.get("message", "")
                    logger.debug(
                        f"Portal error response keys={list(combined_data.keys())} "
                        f"error={err_type!r} message={err_msg!r}"
                    )
                    detail = err_msg or err_type or "Unknown error"
                    raise Exception(f"Portal returned error: {detail}")
