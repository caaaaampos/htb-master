...
App cards provide app-specific guidance to HTB Agent agents. They help agents understand how to operate specific apps more effectively.
...
3. **Automatic Loading**: HTB Agent automatically loads the appropriate app card based on the current package name
...
3. **Absolute path**:
   ```json
   {"com.google.gm": "/usr/share/htb_agent/cards/gmail.md"}
   ```
...
2. **From device**:
   - Open the app
   - Run HTB Agent with debug mode to see the current package name in logs
...
```python
from htb_agent.config_manager import AppCardLoader, config
