...
This module handles downloading, installing, and managing the HTB Agent Portal app
...
VERSION_MAP_GIST_URL = "https://raw.githubusercontent.com/htb_agent/gists/refs/heads/main/version_map_android.json"

PORTAL_PACKAGE_NAME = "com.droidrun.portal"
PORTAL_A11Y_CLASS = "com.droidrun.portal.service.DroidrunAccessibilityService"
PORTAL_IME_CLASS = ".input.DroidrunKeyboardIME"
A11Y_SERVICE_NAME = f"{PORTAL_PACKAGE_NAME}/{PORTAL_A11Y_CLASS}"
PORTAL_IME_ID = f"{PORTAL_PACKAGE_NAME}/{PORTAL_IME_CLASS}"


def portal_content_uri(pkg: str, path: str) -> str:
    """Build a content URI for the configured Portal package."""
    return f"content://{pkg}/{path}"


def portal_a11y_service(pkg: str = PORTAL_PACKAGE_NAME) -> str:
    """Return the accessibility service component for the Portal package."""
    if pkg == PORTAL_PACKAGE_NAME:
        return A11Y_SERVICE_NAME
    return f"{pkg}/{pkg}.service.DroidrunAccessibilityService"


def portal_ime_id(pkg: str = PORTAL_PACKAGE_NAME) -> str:
    """Return the keyboard IME component for the Portal package."""
    if pkg == PORTAL_PACKAGE_NAME:
        return PORTAL_IME_ID
    return f"{pkg}/{PORTAL_IME_CLASS}"
...
def get_compatible_portal_version(
    htb_agent_version: str, debug: bool = False
    ...
    if htb_agent_version in mappings:
        return (mappings[htb_agent_version], download_base, True)

    # Try range match (e.g., "0.4.0-0.4.14": "1.0.0")
    for key, portal_version in mappings.items():
        if "-" in key and _version_in_range(htb_agent_version, key):
...
async def ping_portal_content(device: AdbDevice, debug: bool = False):
    ...
    try:
        state = await device.shell(
            f"content query --uri {portal_content_uri(PORTAL_PACKAGE_NAME, 'state')}"
        )
...
async def set_overlay_offset(device: AdbDevice, offset: int):
    ...
    try:
        cmd = (
            f'content insert --uri "{portal_content_uri(PORTAL_PACKAGE_NAME, "overlay_offset")}" '
            f"--bind offset:i:{offset}"
        )
...
async def toggle_overlay(device: AdbDevice, visible: bool):
    ...
    try:
        ...
        cmd = (
            f'content insert --uri "{portal_content_uri(PORTAL_PACKAGE_NAME, "overlay_visible")}" '
            f"--bind visible:b:{visible_str}"
        )
        await device.shell(cmd)
    except Exception as e:
...
async def setup_keyboard(device: AdbDevice):
    """
    Set up the HTB Agent keyboard as the default input method.
    Simple setup that just switches to HTB Agent keyboard without saving/restoring.
...
async def disable_keyboard(
    device: AdbDevice,
    target_ime: str = PORTAL_IME_ID,
):
    """
    Disable a specific IME (keyboard) and optionally switch to another.
    By default, disables the HTB Agent keyboard.

    Args:
        target_ime: The IME package/activity to disable (default: HTB Agent keyboard)
...
async def setup_portal(
    ...
    current htb_agent SDK version.  Falls back to the latest release if
    ...
    if not is_installed or needs_upgrade:
        ...
        if not success:
            raise RuntimeError(
                ...
                "Run 'htb_agent doctor' for diagnostics."
            )
