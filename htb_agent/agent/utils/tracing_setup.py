...
from htb_agent.config_manager.config_manager import TracingConfig
...
def setup_tracing(
    ...
    if _tracing_initialized:
        ...
        if provider == "langfuse" and agent:
            from htb_agent.telemetry.langfuse_processor import set_current_agent
...
def _setup_phoenix_tracing() -> bool:
    ...
    try:
        from htb_agent.telemetry.phoenix import arize_phoenix_callback_handler
    except ImportError:
        logger.warning(
            "⚠️  Arize Phoenix is not installed.\n"
            "    To enable Phoenix integration, install with:\n"
            "    • If installed via tool: `uv tool install htb_agent[phoenix]`"
            "    • If installed via pip: `uv pip install htb_agent[phoenix]`\n"
        )
        return False
...
def _setup_langfuse_tracing(
    ...
    try:
        ...
        from htb_agent.telemetry.langfuse_processor import (
        ...
        logger.warning(
            ...
            "    • If installed via tool: `uv tool install htb_agent[langfuse]`\n"
            "    • If installed via pip: `uv pip install htb_agent[langfuse]`\n"
...
def record_langfuse_screenshot(
    ...
    try:
        ...
        tracer = trace.get_tracer("htb_agent.screenshot")
        ...
        span = tracer.start_span("htb_agent.screenshot", context=parent_ctx)
        try:
            span.set_attribute("htb_agent.screenshot.image_base64", image_b64)
            span.set_attribute("htb_agent.screenshot.mime_type", mime_type)
        finally:
            span.end()
    except Exception as e:
