...
from htb_agent.agent.usage import track_usage

if TYPE_CHECKING:
    from htb_agent.config_manager.config_manager import LLMProfile
...
def load_llm(provider_name: str, model: str | None = None, **kwargs: Any) -> LLM:
    ...
    if provider_name == "openai_oauth":
        from htb_agent.agent.utils.oauth.openai_oauth_llm import OpenAIOAuth
        return OpenAIOAuth(**{k: v for k, v in kwargs.items() if v is not None})
    if provider_name == "anthropic_oauth":
        from htb_agent.agent.utils.oauth.anthropic_oauth_llm import AnthropicOAuthLLM
        return AnthropicOAuthLLM(**{k: v for k, v in kwargs.items() if v is not None})
    if provider_name == "gemini_oauth_code_assist":
        from htb_agent.agent.utils.oauth.gemini_oauth_code_assist_llm import GeminiOAuthCodeAssistLLM
