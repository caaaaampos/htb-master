...
parsing, and action loop — zero imports from ``htb_agent``.

An external agent can be either:
- A single file: ``htb_agent/agent/external/my_agent.py``
- A package:     ``htb_agent/agent/external/my_agent/__init__.py``
...
def load_agent(name: str) -> Optional[ExternalAgentModule]:
    ...
    """
    try:
        module = importlib.import_module(f"htb_agent.agent.external.{name}")
