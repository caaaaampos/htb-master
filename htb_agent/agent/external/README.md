...
External agents are self-contained modules that receive raw ADB access and run independently from HTB Agent's internal tools.

## Quick Start

1. Add your agent as a file or folder in this directory
2. Configure it in `config.yaml` under `external_agents.<name>`
3. Run with `htb_agent run "task" --agent <name>`
...
- **Zero imports from `htb_agent`** — your agent must be fully self-contained
