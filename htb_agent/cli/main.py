"""
HTB Agent CLI - Command line interface for controlling Android devices through LLM agents.
"""
...
from htb_agent import ResultEvent, DroidAgent
from htb_agent.log_handlers import CLILogHandler, configure_logging
from htb_agent.cli.event_handler import EventHandler
from htb_agent.config_manager import ConfigLoader
from htb_agent.macro.cli import macro_cli
from htb_agent.portal import (
    PORTAL_PACKAGE_NAME,
    download_portal_apk,
...
from htb_agent.telemetry import print_telemetry_message
...
def _setup_cli_logging(debug: bool) -> None:
    """Configure the htb_agent logger with a CLILogHandler."""
...
async def run_command(
    ...
    **kwargs,
    ...
    if config.logging.rich_text:
        cloud_text = Text()
        cloud_text.append("✨ Try HTB Agent Cloud: ", style="bold cyan")
        cloud_text.append(
            "https://cloud.hackthebox.com/sign-in", style="bold blue underline"
        )
        cloud_panel = Panel(
            cloud_text,
            border_style="cyan",
        ...
        console.print("\n✨ Try HTB Agent Cloud: https://cloud.hackthebox.com/sign-in\n")
...
def _print_version(ctx, param, value):
    ...
    version = None
    try:
        version = importlib.metadata.version("htb_agent")
        # print("debug: step 1")
    except Exception:
        pass
    ...
    if not version:
        try:
            from htb_agent import __version__ as pkg_version
...
@click.option(
    ...
    expose_value=False,
    is_eager=True,
    help="Show htb_agent version and exit",
)
def cli():
    """HTB Agent - Control your Android device through LLM agents."""
    pass
...
async def run(
    ...
    try:
        ...
        # Disable HTB Agent keyboard after execution
        ...
        try:
            ...
            click.echo("Failed to disable HTB Agent keyboard")
...
async def _setup_portal(
    ...
    """Internal async function to install and enable the HTB Agent Portal on a device."""
    try:
        if not device:
            devices = await adb.list()
            if not devices:
        ...
        if path:
            ...
            if success:
                console.print(
                    "\n[bold green]Setup complete![/] The HTB Agent Portal is now installed and ready to use."
                )
            else:
                console.print(
                    "[bold red]Setup failed.[/] Run 'htb_agent doctor' for diagnostics."
                )
            return
...
async def setup(
    ...
    """Install and enable the HTB Agent Portal on a device."""
...
async def doctor(device: str | None, debug: bool | None):
    ...
    from htb_agent.cli.doctor import run_doctor
...
@cli.command()
def tui():
    """Launch the HTB Agent Terminal User Interface."""
    from htb_agent.cli.tui import run_tui
