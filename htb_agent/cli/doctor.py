"""HTB Agent Doctor - System health checks and diagnostics."""
...
async def check_sdk_version(debug: bool) -> CheckResult:
    """Check HTB Agent SDK version against latest GitHub release."""
    current = __version__
    latest = None

    for host in GITHUB_API_HOSTS:
        try:
            url = f"{host}/repos/htb_agent/htb_agent/releases/latest"
    ...
    if current_t < latest_t:
        return CheckResult(
            ...
            detail="pip install --upgrade htb_agent",
        )
    else:
...
async def check_config(debug: bool) -> tuple[CheckResult, Any]:
    ...
    from htb_agent.config_manager.loader import OutdatedConfigError
    ...
    try:
        ...
        return (
            CheckResult(
                ...
                detail="See: htb_agent config_example.yaml",
            ),
            None,
        )
    except Exception as e:
        ...
        if installed_t < latest_t:
            ...
            return (
                CheckResult(
                    ...
                    Status.WARN,
                    msg,
                    detail=f"pip install --upgrade htb_agent, then htb_agent setup",
                ),
                installed,
                expected,
                download_base,
...
async def check_portal_state(
    ...
    from htb_agent.tools.android.portal_client import PortalClient
...
async def check_keyboard(device: AdbDevice, debug: bool) -> CheckResult:
    """Check if HTB Agent keyboard IME is available."""
    try:
        output = await device.shell("ime list -s")
        ime_id = portal_ime_id(PORTAL_PACKAGE_NAME)
        if ime_id in output:
...
async def run_doctor(
    ...
    if not is_installed:
        # Auto-fix: run setup
        from htb_agent.cli.main import _setup_portal
