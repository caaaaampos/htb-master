---
name: htb_agent-docs
description: HTB Agent documentation reference. Use when users ask about HTB Agent setup, configuration, SDK usage, CLI commands, device setup, agents, architecture, app cards, credentials, tracing, Docker, migration, structured output, or any HTB Agent "how do I..." questions.
---

# HTB Agent

HTB Agent is an open-source (MIT) framework for controlling Android and iOS devices through LLM agents.
It enables mobile automation using natural language commands.

- **GitHub**: https://github.com/htb_agent/htb_agent
- **Docs site**: https://docs.hackthebox.com
- **License**: MIT
- **Install**: `uv tool install htb_agent` (Google Gemini, OpenAI, Ollama, OpenRouter included by default)
- **Optional extras**: `anthropic`, `deepseek`, `langfuse`
- **Requires**: Python 3.11+, ADB, Portal APK on device

## Architecture

HTB Agent uses a multi-agent architecture coordinated by `DroidAgent`:
...
Source code is at `htb_agent/` (Python package). Key locations:

| Path | Description |
|------|-------------|
| `htb_agent/agent/droid/` | DroidAgent orchestrator |
| `htb_agent/agent/codeact/` | CodeActAgent (direct mode) |
| `htb_agent/agent/scripter/` | ScripterAgent (off-device scripts) |
| `htb_agent/agent/oneflows/` | StructuredOutputAgent |
| `htb_agent/agent/utils/` | Tools, executor, tracing, async utils |
| `htb_agent/tools/` | Device tools (ADB, iOS, portal client) |
| `htb_agent/cli/` | CLI entry point (click-based) |
| `htb_agent/config/prompts/` | Jinja2 prompt templates per agent |
| `htb_agent/config/app_cards/` | App-specific instruction cards |
| `htb_agent/credential_manager/` | YAML-based credential storage |
| `htb_agent/telemetry/` | Phoenix tracing integration |
...
For deeper implementation details, check the source code directly in `htb_agent/`.
